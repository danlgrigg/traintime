...
$("#add-train-btn").on("click", function(event) {
  // Prevent the default form submit behavior; prevents form from submiting when empty
  event.preventDefault();

  // Grabs user input; set variables to store user input data
  ...
  // Uploads train data to the database; push to firebase?
  ...
  // Clears all of the text-boxes; default to blank input field
...
trainData.ref().on("child_added", function(childSnapshot, prevChildKey) {
  ...
  // Store everything into a variable. Grabs input field data and stores into variable
  ...
  // If the first train is later than the current time, sent arrival to the first train time; if then to determine which time displays
  if (maxMoment === trainTime) {
    tArrival = trainTime.format("hh:mm A");
    tMinutes = trainTime.diff(moment(), "minutes");
  } else {
    // Calculate the minutes until arrival using hardcore math; hardcore math?
  ...
  // Add each train's data into the table; display in HTML
